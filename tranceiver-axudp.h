...
public:
	tranceiver_axudp(const std::string & id, seen *const s, work_queue_t *const w, const position_t & pos, const int listen_port, const std::vector<std::pair<std::string, filter *> > & filters, const bool continue_on_error, const bool distribute);
	virtual ~tranceiver_axudp();

	std::string get_type_name() const override { return "AXUDP"; }

	static tranceiver *instantiate(const libconfig::Setting & node, work_queue_t *const w, const position_t & pos, const std::map<std::string, filter *> & filters);
