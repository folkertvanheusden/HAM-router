...
db::db(const std::string & url, const std::string & username, const std::string & password) : url(url), username(username), password(password)
{
	...
	try {
		...
		if (!check_table_exists("APRS")) {
			sql::Statement *stmt = con->createStatement();
			stmt->execute("CREATE TABLE APRS(ts datetime not null, rssi double, snr double, crc int(1) not null, content blob, latitude double, longitude double, distance double, callsign_to varchar(8), callsign_from varchar(8))");
			delete stmt;
		}
	}
	catch(sql::SQLException & e) {
