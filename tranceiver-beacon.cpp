...
tranceiver_beacon::tranceiver_beacon(const std::string & id, seen *const s, work_queue_t *const w, const std::string & beacon_text, const int beacon_interval) :
	tranceiver(id, s, w),
...
void tranceiver_beacon::operator()()
{
	log(LL_INFO, "Beacon: started thread");
		...
		m.message = reinterpret_cast<uint8_t *>(strdup(beacon_text.c_str()));
		m.s       = beacon_text.size();
...
tranceiver *tranceiver_beacon::instantiate(const libconfig::Setting & node_in, work_queue_t *const w)
{
	...
	return new tranceiver_beacon(id, s, w, beacon_text, beacon_interval);
}
